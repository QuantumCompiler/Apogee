...
| `.github/workflows/ci.yml` | CI: **three stages** plus one PR-only check, each a call into `cicd.sh` (user decisions, 2026-09-19, 2026-09-20 and 2026-09-22). The check is `version bump`, which fails a pull request whose `CMakeLists.txt` version is already released — since a merge to `stable` publishes, a forgotten bump would otherwise merge and release nothing, silently. The stages: `clone llama.cpp` (`--clone-llama`: the pin must resolve before anything is built, because llama.cpp ships with the tool) and `test <platform>` running in parallel, then `build <platform>` (one job per target, llama.cpp linked in like every build) once both agree. **The suite runs on a runner again** (user decision, 2026-09-22, reversing 2026-09-20's "remove the tests from the CI/CD"): one job per platform, built **without llama.cpp** (`APOGEE_CMAKE_ARGS=-DAPOGEE_ENABLE_LLAMA=OFF`, the `make no-llama` configuration) so the gate costs minutes rather than rebuilding in full what stage 3 rebuilds — the llamacpp backend's own cases do not run there. `make test` is still the developer's gate before a push. **Every one of those jobs is required for a pull request** (user decision, 2026-09-19, superseding the 2026-08-24 "macos-arm64 only" gate: no `continue-on-error` anywhere, and the branch protection rule on `stable` lists them by job name) — **and nothing else** (user decision, 2026-09-19: CI builds the executable for each platform; formatting, clang-tidy, install parity and the clean-room build are local gates — `make format-check`, `make lint`, the `cli.install_parity` case the suite runs on every platform build, and `cicd.sh --fresh`). Each runner installs what its image lacks — libcurl headers and `python3-venv` on Linux; on Windows the build is a **MinGW-w64** one (user decision, 2026-09-19) in the MSYS2 shell `msys2/setup-msys2` prepares with the toolchain (GCC in UCRT64 on x64, clang in CLANGARM64 on arm64), cmake, ninja, pkgconf and the Schannel-built `curl-winssl` — and passes `--no-defer`, so a misdetected host fails instead of "deferring" to a success. `.github/workflows/release.yml` | Releases: a merge to `stable` — or a pushed `v*` tag — passes the `gate` job and then builds all five targets on native runners through `cicd.sh` (no suite run — CI already ran it on the same commit, 2026-09-22), verifies the staged binary **runs**, and publishes one archive per target to a GitHub Release. A thin caller into `cicd.sh` — it lives outside `lib/src/cli/` only because GitHub requires workflows at `.github/workflows/`. |
...
Feature releases are `v0.x.0`; patch releases are `v0.x.y`. Development happens on a branch named for the release being built and merges into `stable` when that release is done — **and that merge IS the release** (user decision, 2026-09-22): the push to `stable` builds and publishes whatever version `CMakeLists.txt` names, if it has not been published already — `v0.1.0` was tagged on 2026-09-22, so the next branch is `v0.1.1`. ROADMAP.md tracks each release at the theme level; MILESTONES.md records what actually shipped in detail. The version itself lives in exactly one place: the `project(... VERSION x.y.z)` call in `lib/src/cli/CMakeLists.txt`, which flows into `apogee version`. A **tag** is what cuts a release — the policy is [below](#release-and-install-infrastructure), the procedure is in [DEVELOPER.md](DEVELOPER.md#cutting-a-release).
...
Development happens on a version-named branch and merges into `stable` when the release is complete, **and that merge publishes the release** (user decision, 2026-09-22). [`.github/workflows/release.yml`](../../../.github/workflows/release.yml) triggers on a push to `stable` *or* on `push: tags: v*`; both meet in one `gate` job, which reads `project(... VERSION x.y.z)` from `lib/src/cli/CMakeLists.txt`, stops the run when that version already has a release, and fails it when a pushed tag's name disagrees with it. Past the gate it builds each of the five targets on its own native runner **through `lib/scripts/cicd.sh`** — the same script CI and local builds use, so a release binary cannot diverge from a tested one — proves each staged binary actually runs, and then publishes a GitHub Release carrying one archive per target.
...
The tag is created by `publish`, in the same `gh release create --target <sha>` that makes the release, so nothing earlier in the workflow pushes one and a failed build strands nothing. A tag pushed by `GITHUB_TOKEN` cannot trigger another workflow — that is why the merge path lives inside this workflow rather than auto-tagging and relying on the tag trigger, and it is what keeps the repo free of release secrets.
...
Cutting one is therefore *merging the PR*; `make -C lib/src/cli release VERSION=x.y.z` remains the escape hatch for re-cuts and off-tip tags, preflighting the tree, branch, version and tag before it pushes. The step-by-step procedure, the dry run, and how to re-cut a bad release are in [DEVELOPER.md](DEVELOPER.md#cutting-a-release).
