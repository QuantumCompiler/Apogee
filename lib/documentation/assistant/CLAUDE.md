...
**Rule.** A secret — the admin bearer token today, a stored API key next — has no type that anything serializes. What a route, an event, or a log line can carry is a **view** that structurally lacks the secret field: `backend_view` has `api_key_set` and no `api_key`; nothing at all turns the token into JSON. A secret is accepted **body-only** and, when literal, from a **loopback peer only**, judged from the socket's own address (`HttpRequest::remote_address`) and never from a header a client can set. On disk it is `0600` before it is renamed into place.
