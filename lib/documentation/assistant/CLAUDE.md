...
**Enforcement.** The `harness.layering` ctest case (`tests/layering.cmake`) greps every source in `harness/`, `agentloop/`, `agent/`, `secrets/`, `tools/` and `mcp/` for the forbidden include and fails naming the file. It refuses to run against an empty source list for any guarded package, so it cannot pass vacuously.
