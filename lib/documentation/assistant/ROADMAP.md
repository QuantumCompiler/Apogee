...
- Local-model depth — **split into four at grooming (2026-09-06)**. **Model operations shipped 2026-09-07** *([Milestone N](MILESTONES.md#milestone-n--model-operations))*: one shared role resolver used by every surface, the `apogee models list/info/status` suite, and a real GGUF header reader that replaced a magic-bytes check which had been reporting "model loads" for files that cannot load. Still pending: per-family behaviour profiles (the quirk layer — **needs local weights on the build machine**, and Gemma is still missing), open acquisition (no forbidden models; sources = Hugging Face direct **and** Ollama pulls), and local multimodal (llama.cpp mtmd — the cloud half already ships).
