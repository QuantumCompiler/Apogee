...
- **Provider credential store** (19b, gated on the admin plane — gate satisfied): `0600` slots per API-billing provider, `apogee auth`, one key resolver with env snapshotted at start, loopback-only secret intake. Carries three `[user]` calls: keychain vs file, whether config `api_key` stays a fallback, and where git-forge slots land
