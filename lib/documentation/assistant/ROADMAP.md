...
- Local-model depth — **split into four at grooming (2026-09-06)**: per-family behaviour profiles (the quirk layer, characterized from real weights), model operations (one shared role resolver + the `models` suite), open acquisition (no forbidden models; sources = Hugging Face direct **and** Ollama pulls), and local multimodal (llama.cpp mtmd — the cloud half already ships).
