...
- [x] **`quantize` brings the projector.** The quantized copy's directory gets its source's projector as a hard link (a copy across filesystems) with its record, so the smaller model still reads images.
