...
2. **Dual-path cloud access, four vendors** *(revised 2026-08-24)* — Anthropic, OpenAI, Google, and Ollama cloud models, each reachable through its official CLI (**subscription plans**: claude / codex / gemini / ollama, driven as persistent child processes with token-level streaming per the design notes in [`backlog/claude-cli-backend.md`](../backlog/claude-cli-backend.md)) and, where the vendor offers one, its direct HTTPS API (**API billing plans**). Both plans must work; the choice is per backend entry. Ommi had only the Anthropic API and a per-request claude shell-out.
