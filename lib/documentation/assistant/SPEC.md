...
2. **Dual-path cloud access, four vendors** *(revised 2026-08-24)* — Anthropic, OpenAI, Google, and Ollama cloud models, each reachable through its official CLI (**subscription plans**: claude / codex / gemini / ollama, driven as child processes per the design notes in [MILESTONES.md](MILESTONES.md#appendix--vendor-cli-design-notes-carried-forward-from-the-claude-cli-backend-item) — a *persistent* child with token-level streaming **where the vendor's CLI supports it**, which is not universal: `ollama` offers no event stream and no turn protocol, so that backend spawns per turn and streams coarsely, recorded as a limitation rather than papered over *(established by characterization, 2026-09-06)*) and, where the vendor offers one, its direct HTTPS API (**API billing plans**). Both plans must work; the choice is per backend entry. Ommi had only the Anthropic API and a per-request claude shell-out.
