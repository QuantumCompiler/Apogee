...
2. **Dual-path cloud access, four vendors** *(revised 2026-08-24)* — Anthropic, OpenAI, Google, and Ollama cloud models, each reachable through its official CLI (**subscription plans**: claude / codex / gemini / ollama, driven as child processes per the design notes in [MILESTONES.md](MILESTONES.md#appendix--vendor-cli-design-notes-carried-forward-from-the-claude-cli-backend-item) — a child process per vendor, with the shape each CLI actually allows rather than one assumed for all four — **established by characterizing every one of them against a real login, complete 2026-09-06**. Only `claude` offers a stdin turn protocol, so only that backend keeps a *persistent* child; `codex` and `gemini` spawn per turn but resume a server-side session, so history is never re-sent; `ollama` has neither, so it spawns per turn *and* re-sends the conversation. Streaming granularity varies the same way — token deltas on `claude` and `gemini`, whole messages on `codex`, no event stream at all on `ollama` — and each limitation is recorded rather than papered over) and, where the vendor offers one, its direct HTTPS API (**API billing plans**). Both plans must work; the choice is per backend entry. Ommi had only the Anthropic API and a per-request claude shell-out.
...
- **Training/distillation** *(confirmed direction 2026-08-24; scheduled for v0.1.0 2026-09-19)*: fine-tuning local models on their full-weight SafeTensors files, cloned from Ommi's orchestration shape — a C++ orchestrator over Python trainer subprocesses in a virtual environment Apogee owns, never the system Python. The floor (datasets, kits, the Python boundary), the run (train, the eval gate, promotion to a versioned GGUF, rollback) and the orchestration layer (multi-stage pipelines under a cumulative gate, regimes distilling a teacher across kits, and the scheduler-invoked cycle with its anchor gate and circuit breaker -- never a daemon) all shipped 2026-09-19; the track is complete. Training control is CLI-only over HTTP; the reads are served.
...
- **A vendor CLI is spawned, never opened.** *(Promoted to a principle 2026-08-25; it binds every subscription-plan backend, not just Claude's.)* When Apogee drives a vendor's official CLI, it **never reads that CLI's credentials** — it does not open `~/.claude` or its siblings, does not implement an OAuth flow, and does not parse session files off disk. The CLI authenticates; Apogee only spawns it. Crash recovery goes through the vendor's own `--resume`, which is exactly why disk-reading is never needed. Apogee also **never modifies or bundles the vendor binary**: it is resolved from `PATH` (or an explicit config path), the user installs and logs in themselves, and no built-in auth method is suppressed. This is the line separating "the user runs their vendor CLI" from "a third-party product routes subscription credentials," and it is not negotiable per-backend.
