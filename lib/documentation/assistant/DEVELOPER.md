...
Reserved packages carry a documented header and no code. They exist so every later backlog item has one obvious home, and `tests/packages_test.cpp` includes each one so a broken include path is caught the day it breaks rather than months later. `embedstore/` graduated out of that set in Milestone Q; its umbrella header now names the package's real headers, so the same test keeps checking the same thing.
