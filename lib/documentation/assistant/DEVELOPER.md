...
| `model_profile.h/.cpp` | What Apogee knows about a **family** of local model, and the ladder that resolves it: an explicit `chat_template:` > the GGUF's own `general.architecture` > a name hint > nothing. Rung 2 above rung 3 on purpose — an architecture is a fact in the file, a name is a string somebody chose. Every profile carries a `verified` flag and an **evidence line naming the model that was run**; only `gemma3` and `qwen3` are verified, and a test asserts that list. A known profile's empty reasoning list means "emits none" (a result); nullptr means uncharacterised (permissive). |
