...
`cli.no_vendor_credentials` (`tests/no_vendor_credentials.cmake`) greps every source for signs of reading a vendor CLI's credential store — their dot-directories (`.claude/`, `.gemini/`, `.codex/`), `credentials.json`, `google_accounts.json`, `oauth_creds.json`, `apiKeyHelper`, OAuth, the keychain. `~/.ollama/` is deliberately **not** listed, and the file says why: that directory is a *model* store, and reading it is a supported source in the model-acquisition item. It enforces the SPEC principle *a vendor CLI is spawned, never opened*, and it is mechanical rather than reviewed because the shortcut it forbids is genuinely tempting: when a `--resume` fails, reading the vendor's session file *would* recover the conversation. Documentation lines are exempt so the rule can be explained where it is enforced.
