...
| `backends/` | Every provider plus the infrastructure they share: `mock`, `anthropic`, `openai`, `google`, `llamacpp`, `claude_cli`, and `ollama_cli`; the HTTP client and SSE parser for the direct-API ones; the `llama_runtime` seam, chat templates and token arithmetic for the local one; the JSONL framer and typed event union for the vendor-CLI ones. |
