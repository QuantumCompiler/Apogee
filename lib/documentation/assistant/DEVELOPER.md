...
| `mux.h/.cpp` | The route table: method + path → handler method, `{id}` capture, 404/405 in the error shape. `Mux::routes()` exposes the table; `tests/http_api_conformance.cmake` reads it off the source. |
...
| `serve.h/.cpp` | The bind policy (`is_loopback_host`, `bind_refusal` — fail-closed behind `--allow-remote`) and `run_server`: cpp-httplib behind the mux, the sweeper thread, SIGINT/SIGTERM → `stop_serving()`. **The only translation unit that may reference `listen()`/`accept()`**; the symbol check allow-lists `serve.cpp.o` by name. |
...
| `httpserver/serve_test.cpp` | The bind policy table (every loopback spelling; fail-closed otherwise) and the route table. |
