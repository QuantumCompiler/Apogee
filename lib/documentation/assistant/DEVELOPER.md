...
| `agentloop/` | **The shared loop.** `run()` drives model→tool→model behind a Reporter, plus `ask_user`, history compaction, and transient splicing. `rag.h/.cpp` builds the retrieved-excerpt prefix that rides the transient path — so context reaches the model and never the saved session. Includes nothing from `backends/` — enforced. |
...
| `agentloop/rag.h/.cpp` | Retrieval → a transient system prefix. The excerpts are **framed as retrieved excerpts** explicitly: without that framing a model reads an injected document as something the user said, and the more relevant the retrieval the more confidently it answers about the document instead of the question. A missing collection or a store that throws sets `error` and returns no prefix — answering without context beats refusing to answer. |
| `commands/embed.h/.cpp` | `apogee embed ingest / query / list / info / delete`. Collections live at `~/.apogee/embeddings/<name>.db` and the name must be a plain one — the path is built, never accepted. The first ingest of a name **registers it under `embeddings:`** through `edit_config_file`, recording the chunking used; later ingests read that chunking back as their default, and a flag wins for one run without rewriting the file. A config that will not load or will not take the edit costs the registration and says so on stderr — never the ingest, which is already on disk. |
