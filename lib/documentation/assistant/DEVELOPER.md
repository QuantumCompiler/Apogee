...
| `commands/models_test.cpp` | The listing surface: that the ROLES column comes from the shared resolver, that a cloud backend is not reported as a missing file, and that an unreadable header states its reason **on the failure line** rather than anywhere in the body. |
