...
**Seam + files.** lib/src/backends/anthropic.h/.cpp, lib/src/backends/sse_parser.h/.cpp (shared SSE frame parser), lib/src/backends/http_client.h/.cpp (thin wrapper over the chosen HTTP lib with retry/backoff, shared by all cloud backends), IR↔Anthropic wire translation unit (system extraction, tool blocks, thinking blocks, image sources), lib/test/backends/anthropic_test.cpp (recorded-fixture SSE streams incl. frames split across read boundaries — no network).
