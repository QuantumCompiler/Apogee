...
**Seam + files.** lib/src/cli/json_reporter.cpp (Reporter→JSONL adapter, sibling of cliReporter), `--output-format`/`--input-format` flags on complete.cpp and chat.cpp, a stdin JSONL reader for driven chat sessions (mutex-guarded single writer on the child side mirrors the notes' process model), an event-schema reference doc shipped with the repo, lib/test/cli/machine_mode_test.cpp (exec-style: drive the real binary over pipes with a mock backend; fixture round-trips; an adversarial-chunk consumer test proving events parse identically at any read granularity; the lsof no-LISTEN assertion).
