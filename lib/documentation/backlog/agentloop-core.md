...
**Seam + files.** lib/src/agentloop/loop.h/.cpp (Run, StreamTurn, Options with injected AskFn/EmbedFn/Reporter seams), lib/src/agentloop/reporter.h, lib/src/agentloop/content.cpp (compaction, EstimateTokens ~len/4 flagged estimated, spliceTransient), lib/src/agentloop/question.cpp (ask_user schema, validation, EncodeAnswers, rollback), lib/src/agent/dispatch.cpp (tool registry, DispatchTool with handled=false fallthrough, permission gate; non-interactive ask resolves to deny), lib/src/cli/complete.cpp (--tools + --search/--mcp/--native tool-mode flags, mutually exclusive), lib/test/agentloop/ (scripted-provider conformance suite).
