...
**Seam + files.** lib/src/cli/source/knowledge/ (record.h/.cpp schema/validate/anonymize, embedded capture+refine prompts and schema via #embed/codegen, store over embedstore, raw archive), lib/src/cli/source/graph/ (extraction contract, BuildMulti over injected extract/embed closures, materializeRecords, DetectCommunities), lib/src/cli/source/embedstore/graph*.cpp (kg_* tables, GraphExpand, communities, dedupe), lib/src/cli/source/commands/knowledge.cpp + graph.cpp, lib/src/cli/source/httpserver/knowledge.cpp + graph.cpp slices; all clerks injected closures so both layers test model-free; lib/src/cli/tests/knowledge/ + lib/src/cli/tests/graph/.
