...
**Scope note.** Phase 4, item **24a**, the first of the local-agent-tools track; gated on nothing. Out of scope: taint tracking (asking only once private data has been read), per-path file permissions, and guarding MCP servers' own network use, which happens in their processes, not in Apogee's.
