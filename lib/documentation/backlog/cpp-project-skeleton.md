...
- Six-target portability from day one (linux/macos/windows × x64/arm64, user decision 2026-08-24): OS-specific mechanisms (process spawning, PTY/terminal control, file modes, socket peer checks) go behind a small platform seam (lib/src/cli/source/platform/) rather than inline #ifdefs; POSIX-only test mechanisms named in later items (forkpty, lsof, tcflush) get per-platform equivalents or recorded per-item skips. CMake ships one preset per target, named exactly after the six targets — cicd.sh selects presets by those names
...
**Seam + files.** CMakeLists.txt (root), cmake/ (presets, toolchain, dependency helpers), lib/src/cli/source/main.cpp (root command, subcommand self-registration, --config persistent flag, chosen CLI framework), lib/src/cli/source/harness/ lib/src/cli/source/backends/ lib/src/cli/source/agentloop/ lib/src/cli/source/commands/ lib/src/cli/source/embedstore/ lib/src/cli/source/httpserver/ lib/src/cli/source/mcp/ (empty library-target stubs), lib/src/cli/tests/CMakeLists.txt + lib/src/cli/tests/smoke_test.cpp (per-package test dirs mirroring source/), third_party/llama.cpp (pinned submodule/FetchContent, never edited), .clang-format, .clang-tidy (ownership/smart-pointer checks: cppcoreguidelines-owning-memory, cppcoreguidelines-no-malloc, modernize-make-unique/make-shared — scoped to lib/src, third_party excluded), Makefile (thin wrapper: build/test/install), CI config stub, lib/documentation/assistant/DEVELOPER.md seeded with the target map and how-to-add-a-dependency notes.
...
- 2026-08-24 — App-rooted source layout (user decision): `lib/src/<app>/`, each app with `source/` and `tests/` as its first level. The CLI application is `lib/src/cli/` — packages (harness, backends, agentloop, commands, …) live under `source/`, with `main.cpp` at its root and `tests/` mirroring the package dirs. The GUI later lands as sibling apps `lib/src/darwin|linux|windows` (down the road, not yet planned). All backlog seam paths were rewritten to this layout the same day.
