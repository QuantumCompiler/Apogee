...
**Scope note.** **Gate satisfied 2026-09-12** — the embedders shipped in [Milestone R](../assistant/MILESTONES.md#milestone-r--embedding-clients): `EmbeddingCapable` on the OpenAI, Gemini and llama.cpp providers, discovered through `Harness::embedder_for`, with `embedding_dimensions()` per provider and the dimension fixtures in `tests/support/embedding_fixtures.h` for the per-store binding tests. Read Milestone R before designing against them — in particular the note on what the scripted llama runtime cannot see.
