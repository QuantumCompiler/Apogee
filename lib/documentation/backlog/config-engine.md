...
**Seam + files.** lib/src/harness/config.h/.cpp (Config, BackendConfig structs, LoadConfig, expandEnv), lib/src/harness/config_template.cpp (SaveConfigTemplate), lib/src/harness/config_edit.h/.cpp (AppendBackend/DeleteBackend/SetDefault*/FormatConfig/Get section-scoped text-surgery helpers), lib/src/cli/config_cmd.cpp, assets/config.yaml (checked-in starter sample with commented examples incl. a fill-in user-supplied embedder), lib/test/harness/config_test.cpp + config_edit_test.cpp (golden-file byte-diff suite).
