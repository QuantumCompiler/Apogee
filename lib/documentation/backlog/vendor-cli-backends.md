...
**Scope note.** gated on [claude-cli-backend.md](claude-cli-backend.md) shipping (ring convention: also assumes the complete v0.1.0 set). Out of scope: the vendors' direct-API backends (anthropic-backend, openai-google-backends already own those); local Ollama model *pulls* (that is a model-source concern owned by the models area, not a backend).
