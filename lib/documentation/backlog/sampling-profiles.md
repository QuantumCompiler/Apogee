...
**Scope note.** Item **26h**; gated on nothing pending (25b shipped 2026-09-25: the chain to extend is `make_sampler` in `llama_real.cpp`, built with llama.h's own sampler API -- `common`'s sampler is not used). Out of scope: learning settings per task, and dynamic temperature.
