...
- **One sampler chain.** The grammar (24b's lazy tool grammar, [25f](local-structured-output.md)'s schema grammar) and the [reasoning budget](thinking-control.md) share the chain these settings build; `common_sampler` (`common/sampling.h`) is the upstream implementation and is linked with 24b.
...
- 2026-09-25 — After 24b, which links `common`'s sampler and puts a grammar into the chain.
...
**Scope note.** Phase 4, item **25h**; build after 24b. Out of scope: learning settings per task, and dynamic temperature.
