...
**Seam + files.** lib/src/cli/source/harness/provider.h (LLMProvider ABC + capability interfaces), lib/src/cli/source/harness/types.h/.cpp (ChatMessage, MessageContent string-or-parts, Tool, ToolCall, ChatRequest/Response with TransientStart/TransientLen/SideRequest, RAGMeta, errors), lib/src/cli/source/harness/behavior.h (ModelBehavior plain data + ModelBehaviorFor), lib/src/cli/source/harness/harness.h/.cpp (registry + SimpleRouter), lib/src/cli/source/harness/context_windows.cpp (compiled model→window fallback table — single owner), lib/src/cli/source/backends/mock.cpp (MockProvider), lib/src/cli/tests/harness/.
...
**Guardrail(s).** MockProvider contract tests pin the interface; IR round-trip serialization tests; a test asserting transient messages never appear in persisted/serialized history; a check that lib/src/cli/source/harness has no include of lib/src/cli/source/backends.
