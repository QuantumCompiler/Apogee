...
**Scope note.** Item **26b**; gated on nothing, though running helpers beside a large model assumes [26a](context-fit-defaults.md)'s memory. Out of scope: automatic selection of which model plays which role, and cloud backends as helpers beyond what the resolver already allows (any backend may be pointed at).
