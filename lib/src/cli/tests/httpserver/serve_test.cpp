...
TEST_CASE("the route table is the documented one", "[httpserver][mux]") {
    ...
    CHECK(has("PUT", "/v1/admin/allowed-hosts/{id}"));
    CHECK(has("DELETE", "/v1/admin/allowed-hosts/{id}"));
    CHECK(has("GET", "/v1/admin/training/status"));
    CHECK(has("GET", "/v1/admin/training/runs"));
    CHECK(has("GET", "/v1/admin/training/runs/{id}"));
    CHECK(has("GET", "/v1/admin/training/versions"));
    CHECK_FALSE(has("POST", "/v1/admin/training/runs"));
    ...
    CHECK(has("DELETE", "/v1/admin/graph/{id}"));
    CHECK(has("PUT", "/v1/admin/embeddings/{id}/graph"));
    CHECK(has("POST", "/v1/admin/graph/{id}/communities"));
    CHECK(has("GET", "/v1/admin/graph/{id}/communities"));
    CHECK(has("POST", "/v1/admin/graph/{id}/dedupe"));
    CHECK(has("GET", "/v1/admin/graphs"));
    CHECK(has("POST", "/v1/admin/graphs"));
    CHECK(has("GET", "/v1/admin/graphs/{id}"));
    CHECK(has("PUT", "/v1/admin/graphs/{id}"));
    CHECK(has("DELETE", "/v1/admin/graphs/{id}"));
    CHECK(has("POST", "/v1/admin/knowledge/capture"));
    CHECK(has("POST", "/v1/admin/knowledge"));
    CHECK(has("POST", "/v1/admin/knowledge/refine"));
    CHECK(has("POST", "/v1/admin/knowledge/reindex"));
    CHECK(has("GET", "/v1/admin/knowledge"));
    CHECK(has("GET", "/v1/admin/knowledge/{id}"));
    CHECK(has("PATCH", "/v1/admin/knowledge/{id}"));
    CHECK(has("DELETE", "/v1/admin/knowledge/{id}"));
